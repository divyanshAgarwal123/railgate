...
flowchart LR
    ...
    Screen -->|clean| Gate{Atomic budget reservation}
    Gate -->|would breach cap| Pending[blocked_pending_approval]
    Gate -->|within cap| Reserve[(In-flight reservation)]
    Reserve --> RZP[Razorpay test-mode\norder + payment link]
    Pending -. human approves via CLI .-> Reserve
...
3. **`executed`** — a real Razorpay order + payment link, and a balanced double-entry
   authorization posting (`session:<id>` debited, `merchant:<id>` credited). This records an
   issued payment request, not a captured payment; settlement verification is explicitly out
   of scope below.
...
| `src/governor.ts` | Atomic check-and-reserve spend gate (`requireCheckoutApproval`) and the injection screen (`screenText`). An immediate SQLite transaction makes concurrent checkouts serialize before the external call. |
...
| `src/mcp-server.ts` | MCP tools (`list_products`, `get_product`, `checkout`, `execute_approved`) over stdio. Approval itself is deliberately absent; it lives in the human CLI. |
...
- **`checkout_pending`** — one row per checkout intent. Within-cap requests enter `reserved`
  before Razorpay is called; over-cap requests move `pending` → `approved` → `reserved` →
  `executed`. Approval is single-use.
- **`ledger_txns` / `ledger_legs`** — only issued Razorpay checkout authorizations land here.
  Blocked or pending attempts live in `audit_log` / `checkout_pending`. This is not presented
  as a payment-settlement ledger; capture verification needs webhooks and reconciliation.

## Why the gate is code, not a model call

The spend ceiling and the injection screen are plain deterministic logic — an atomic
accounting check and a regex list — not an LLM asked to judge whether a purchase looks safe. That's
...
- **The regex screen is not claimed as a complete prompt-injection detector.** Rephrasing can
  evade regex and benign text can match it. The money boundary does not depend on the screen:
  callers cannot provide price or merchant, and every amount is checked against the atomic
  reservation gate using server-side catalog data.
